...
A JSON string which stores meta data. This value is NULL by default, need to
add MetaData config to enable it.
...
Matches values by arrival intervals.

This match is mainly intended for dropping too frequent arrival values focedly
at server side. This plugin stores previous arrival times for each values and
match the interval against the interval range which is specified in this
configuration. The first arrival value just after launching collectd is always
unmatched.
...
=item B<Min> I<Seconds>

Minimum interval to match in seconds.

=item B<Max> I<Seconds>

Max interval to match in seconds.

=item B<Invert> I<false|true>

When set to true, the result of the match is inverted.

=item B<UpdateTimestamp> I<"always|match|unmatch">

When set to "match" or "unmatch", the stored timestamps of each values are
updated only when the matching result is matched or unmatched after matching.
Otherwise the timestapms are always updated after matching is performed.
The default is "always".

=item B<Expire> I<Seconds>

Expire the stored timestamps from the memory when a value doesn't arrive more
than the interval which is specified by this option. Because this plugin stores
all timestamps for each values, some of them may become garbage when the
identifiers are changed or clients are removed.
...
Example:
 ...
 <Match "interval">
   Max 300
   UpdateTimestamp "unmatch"
   Expire 86400
 </Match>

When this configuration is specified with 'B<Target "stop">', the values which
within 5 minutes from last arrival will be dropped. In this case the timestamps
shouldn't be updated until the interval exceeds the B<Max> value,
B<UpdateTimeStamp> is set to B<unmatch>.
The previous timestamps will be expired when the value doesn't come for more
then one day.

=item B<ipaddress>

Matches values by sender's IP address. The list of IP addresses to match is
provided by an external plain text file.
...
The path of a file which contains IP addresses to match.
...
Example:
 ...
 <Match "ipaddress">
   FilePath "/etc/collectd.d/ipaddress"
   Invert true
 </Match>
